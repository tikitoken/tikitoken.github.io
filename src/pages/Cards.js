...
function Cards() {
  ...
  useEffect(() => {
    ...
    const wr = getMetamaskWallet().then(w => {
      ...
      w.getAddress().then(address => {
        contract.balanceOf('0xaA7D7470683a97e9e923531a4A0B6F30CA5BcCbb').then(balance => {
          setBalance(parseInt(balance._hex, 16)/1e18)
          setContract(contract)
        })
        contract.withdrawableDividendOf('0xaA7D7470683a97e9e923531a4A0B6F30CA5BcCbb').then(withdrawable => {
          setWithdrawable(parseInt(withdrawable._hex, 16)/1e18)
        })
        contract.withdrawnDividendOf('0xaA7D7470683a97e9e923531a4A0B6F30CA5BcCbb').then(withdrawn => {
        ...
        <Card>
          <CardBody>
            ...
            <p className="text-gray-600 text-2xl dark:text-gray-300 text-center">
              Your currently claimable BNB balance from the recovery protocol is:<br/><br/><span className="text-yellow-300 font-bold">{withdrawable !== null ? withdrawable : 'Loading...'} BNB</span><br/><br/>Click <button onClick={contract !== null ? () => contract.withdrawDividend() : null} className="text-purple-500" >HERE</button> to claim your pending balance.
            </p>
          </CardBody>
        </Card>
